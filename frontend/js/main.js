document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW: State for tracking changes in Import Mode ---
    let createdRecords = [];
    let updatedRecords = new Map(); // Use Map for efficient updates by ID
    let deletedRecordIds = new Set(); // Use Set for efficient add/delete
    ...
    const syncDataButton = document.getElementById('sync-data-button'); // --- NEW ---
    const syncStatus = document.getElementById('sync-status'); // --- NEW ---
    ...
    const modalDeleteButton = document.getElementById('modal-delete-button'); // --- NEW ---
    ...
    if (modalDeleteButton) modalDeleteButton.addEventListener('click', handleModalDelete);
    ...
    async function handleAddRecord(e) {
        ...
        if (currentDataMode === 'direct') {
            ...
            const selectedBatch = addRecordBatchSelect.options[addRecordBatchSelect.selectedIndex];
            
            const newRecord = {
                ...recordData,
                id: tempId,
                batch_name: selectedBatch ? selectedBatch.textContent : 'Unknown',
                event_names: [],
                file_name: 'N/A',
            ...
            allImportedRecords.push(newRecord);
            
            addRecordSuccessMessage.textContent = 'Record cached locally. Click "Sync Data" to save to the server.';
            addRecordForm.reset();
            updateDashboardStats();
    ...
    async function handleModalSave() {
        ...
        const updatedData = {
            id: isNaN(parseInt(recordId)) ? recordId : parseInt(recordId),
        ...
        if (currentDataMode === 'direct') {
            ...
            if (recordIndex > -1) {
                ...
                const createdIndex = createdRecords.findIndex(r => r.id == recordId);
                if (createdIndex > -1) {
                    createdRecords[createdIndex] = allImportedRecords[recordIndex];
                } else {
                    updatedRecords.set(updatedData.id, allImportedRecords[recordIndex]);
                }
                
                updateUnsyncedChangesCount();
                statusContainer.innerHTML = `<p class="text-green-600">Record ${recordId} updated in cache. Sync to save.</p>`;
                editRecordModal.classList.add('hidden');
                refreshCurrentView();
            } else {
                 statusContainer.innerHTML = `<p class="text-red-500">Error: could not find record in cache to update.</p>`;
            }
        }
    }
    ...
    async function handleModalDelete() {
        ...
        if (currentDataMode === 'direct') {
            statusContainer.innerHTML = `<p class="text-blue-600">Deleting record ${recordId}...</p>`;
            try {
                await deleteRecordApi(recordId);
                statusContainer.innerHTML = `<p class="text-green-600">Successfully deleted record ${recordId}!</p>`;
                editRecordModal.classList.add('hidden');
                refreshCurrentView();
            } catch (error) {
                 statusContainer.innerHTML = `<p class="text-red-500">Error deleting record: ${error.message}</p>`;
            }
        } else { // Import mode
            const recordIndex = allImportedRecords.findIndex(r => r.id == recordId);
            if (recordIndex > -1) {
                allImportedRecords.splice(recordIndex, 1);
                
                const createdIndex = createdRecords.findIndex(r => r.id == recordId);
                if (createdIndex > -1) {
                    createdRecords.splice(createdIndex, 1);
                } else {
                    deletedRecordIds.add(parseInt(recordId));
                    if (updatedRecords.has(parseInt(recordId))) {
                        updatedRecords.delete(parseInt(recordId));
                    }
                }

                updateUnsyncedChangesCount();
                statusContainer.innerHTML = `<p class="text-green-600">Record ${recordId} marked for deletion. Sync to save.</p>`;
                editRecordModal.classList.add('hidden');
                refreshCurrentView();
            }
        }
    ...
    async function handleAllDataFileSelect(pageOrUrl = 1) { 
        ...
        if (currentDataMode === 'direct') {
            ...
            if (typeof pageOrUrl === 'string') { 
                ...
                currentAllDataParams = { batch: batchId, page: 1 };
                if (fileName && fileName !== 'all') { 
                    currentAllDataParams.file_name = fileName; 
                } 
                params = new URLSearchParams(currentAllDataParams).toString();
                params = `${API_BASE_URL}/api/records/?${params}`
            } 
            try { 
    ...
    async function openEditModal(recordId) {
        ...
        if (currentDataMode === 'direct') {
             ...
             eventsContainer.innerHTML = '<p class="text-gray-500 text-sm">Event assignment is disabled in Import Mode.</p>';
        }
    ...
    async function handleSyncData() {
        syncStatus.innerHTML = `<p class="text-blue-600">Syncing changes with the server...</p>`;
        syncDataButton.disabled = true;

        const payload = {
            created: createdRecords,
            updated: Array.from(updatedRecords.values()),
            deleted: Array.from(deletedRecordIds)
        };

        try {
            const result = await syncData(payload);
            syncStatus.innerHTML = `<p class="text-green-600">Sync successful! Created: ${result.created}, Updated: ${result.updated}, Deleted: ${result.deleted}. Refreshing local data...</p>`;
            
            resetChangeTracking();
            updateUnsyncedChangesCount();

            allImportedRecords = await getAllRecords();
            ...
            syncStatus.innerHTML = `<p class="text-red-500">Sync failed: ${error.message}</p>`;
        } finally {
    ...
    function updateUnsyncedChangesCount() {
        const count = createdRecords.length + updatedRecords.size + deletedRecordIds.size;
        const countElement = document.getElementById('unsynced-changes-count');
        if (count > 0) {
    ...
    function refreshCurrentView() {
        const activePageId = document.querySelector('section:not(.hidden)').id;
        if (activePageId === 'search-page') {
            handleSearch(null);
        } else if (activePageId === 'alldata-page') {
    ...
    function updateSyncButtonVisibility() {
        const syncContainer = document.getElementById('sync-data-button').parentElement;
    ...
    function filterImportedRecords(params) {
        ...
        for (const key in params) {
            if (typeof params[key] === 'string' && key !== 'batch' && key !== 'file_name' && key !== 'relationship_status') {
        ...
        return allImportedRecords.filter(r => {
            ...
            const voterNoMatch = !lowerCaseParams.voter_no || (r.voter_no && r.voter_no.includes(lowerCaseParams.voter_no));
            ...
            const kromikNoMatch = !lowerCaseParams.kromik_no || (r.kromik_no && r.kromik_no.includes(lowerCaseParams.kromik_no));
            ...
            const batchMatch = !params.batch || r.batch == params.batch;
            const fileNameMatch = !params.file_name || !r.file_name || r.file_name === params.file_name;
            const relationshipMatch = !params.relationship_status || r.relationship_status === params.relationship_status;
    ...
    function renderReadOnlyTable(records) {
        ...
        records.forEach(record => {
            ...
            row.innerHTML = `
                <td class="px-6 py-4 whitespace-nowrap">${record.naam || ''}</td>
                <td class="px-6 py-4 whitespace-nowrap">${record.voter_no || ''}</td>
                <td class="px-6 py-4 whitespace-nowrap">${record.pitar_naam || ''}</td>
                <td class="px-6 py-4 whitespace-nowrap">${record.thikana || ''}</td>
                <td class="px-6 py-4 whitespace-nowrap">${record.relationship_status || ''}</td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <button data-record-id="${record.id}" class="edit-btn text-indigo-600 hover:text-indigo-900">Edit</button>
                </td>
            `;
    ...
    function openFamilyManager() { familyMemberSearchInput.value = ''; familyMemberSearchResults.innerHTML = ''; selectedFamilyMemberDetails.innerHTML = ''; selectedFamilyMemberDetails.classList.add('hidden'); addExistingMemberForm.classList.add('hidden'); existingMemberRelationshipInput.value = ''; addExistingStatus.textContent = ''; addNewFamilyMemberForm.reset(); addNewStatus.textContent = ''; selectedRelativeForFamily = null; switchFamilyTab('existing'); familyManagerModal.classList.remove('hidden'); }
    ...
    async function loadCurrentFamilyMembers(recordId) { currentFamilyMembersList.innerHTML = '<p class="text-gray-500 text-sm">Loading family members...</p>'; try { const data = await getFamilyTree(recordId); currentFamilyMembersList.innerHTML = ''; if (data.results && data.results.length > 0) { data.results.forEach(rel => { const memberDiv = document.createElement('div'); memberDiv.className = 'text-sm p-1.5 flex justify-between items-center'; memberDiv.innerHTML = `<div><span class="font-semibold text-gray-700">${rel.relationship_type}:</span><span class="text-gray-600 ml-2">${rel.relative.naam}</span></div><button data-id="${rel.id}" class="remove-relative-btn text-red-400 hover:text-red-600 text-xs">Remove</button>`; currentFamilyMembersList.appendChild(memberDiv); }); document.querySelectorAll('.remove-relative-btn').forEach(btn => { btn.addEventListener('click', handleRemoveRelationship); }); } else { currentFamilyMembersList.innerHTML = '<p class="text-gray-500 text-sm">No family members added yet.</p>'; } } catch (error) { currentFamilyMembersList.innerHTML = `<p class="text-red-500 text-sm">Error loading family: ${error.message}</p>`; } }
    async function handleFamilyMemberSearch() { const query = familyMemberSearchInput.value.trim(); familyMemberSearchResults.innerHTML = ''; if (query.length < 2) return; try { const data = await searchRecords({ naam__icontains: query, page_size: 5 }); if (data.results && data.results.length > 0) { data.results.forEach(record => { if (record.id === currentRecordForFamily.id) return; const resultBtn = document.createElement('button'); resultBtn.className = 'block w-full text-left p-2 hover:bg-gray-100'; resultBtn.textContent = `${record.naam} (Voter No: ${record.voter_no || 'N/A'})`; resultBtn.onclick = () => { selectedRelativeForFamily = record; familyMemberSearchResults.innerHTML = ''; selectedFamilyMemberDetails.innerHTML = `<p class="text-sm font-medium">Selected: ${record.naam}</p>`; selectedFamilyMemberDetails.classList.remove('hidden'); addExistingMemberForm.classList.remove('hidden'); }; familyMemberSearchResults.appendChild(resultBtn); }); } else { familyMemberSearchResults.innerHTML = '<p class="p-2 text-sm text-gray-500">No results found.</p>'; } } catch (error) { familyMemberSearchResults.innerHTML = `<p class="p-2 text-sm text-red-500">${error.message}</p>`; } }
    async function handleAddExistingMember() { const relationshipType = existingMemberRelationshipInput.value.trim(); if (!selectedRelativeForFamily || !relationshipType) { addExistingStatus.textContent = 'Please select a person and define the relationship.'; addExistingStatus.className = 'text-red-500 text-sm text-center mt-2'; return; } addExistingStatus.textContent = 'Adding...'; addExistingStatus.className = 'text-blue-500 text-sm text-center mt-2'; try { await addFamilyMember(currentRecordForFamily.id, selectedRelativeForFamily.id, relationshipType); addExistingStatus.textContent = 'Successfully added!'; addExistingStatus.className = 'text-green-600 text-sm text-center mt-2'; setTimeout(() => { familyManagerModal.classList.add('hidden'); loadCurrentFamilyMembers(currentRecordForFamily.id); }, 1000); } catch (error) { addExistingStatus.textContent = `Error: ${error.message}`; addExistingStatus.className = 'text-red-500 text-sm text-center mt-2'; } }
    async function handleAddNewMemberFormSubmit(e) { e.preventDefault(); const newMemberNameInput = document.getElementById('new-member-name'); const newMemberRelationshipInput = document.getElementById('new-member-relationship'); const name = newMemberNameInput.value.trim(); const relationship = newMemberRelationshipInput.value.trim(); if (!name || !relationship) { addNewStatus.textContent = 'Name and Relationship are required.'; addNewStatus.className = 'text-red-500 text-sm text-center mt-2'; return; } const newRecordData = { naam: name, voter_no: document.getElementById('new-member-voter-no').value, pitar_naam: document.getElementById('new-member-father').value, matar_naam: document.getElementById('new-member-mother').value, phone_number: document.getElementById('new-member-phone').value, kromik_no: 'N/A' }; addNewStatus.textContent = 'Creating new record...'; addNewStatus.className = 'text-blue-500 text-sm text-center mt-2'; try { const newRecord = await addRecord(newRecordData); addNewStatus.textContent = 'Linking family member...'; await addFamilyMember(currentRecordForFamily.id, newRecord.id, relationship); addNewStatus.textContent = 'Successfully added!'; addNewStatus.className = 'text-green-600 text-sm text-center mt-2'; setTimeout(() => { familyManagerModal.classList.add('hidden'); loadCurrentFamilyMembers(currentRecordForFamily.id); }, 1000); } catch (error) { addNewStatus.textContent = `Error: ${error.message}`; addNewStatus.className = 'text-red-500 text-sm text-center mt-2'; } }
