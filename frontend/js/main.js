document.addEventListener('DOMContentLoaded', () => {
    ...
    let pendingChanges = { updated: {} }; // Object to store unsynced updates { recordId: recordObject }
    let allEventsMap = {}; // To cache event data { eventId: eventObject }
    ...
    if (syncDataBtn) syncDataBtn.addEventListener('click', handleSyncData);
    ...
    function handleLogout() { localStorage.removeItem('authToken'); currentDataMode = 'direct'; allImportedRecords = []; pendingChanges = { updated: {} }; showLogin(); }
    ...
    async function handleImportMode() {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                syncDataBtn.classList.remove('hidden');
                updateSyncStatus();
    ...
    function handleDirectMode() {
        ...
        appContainer.classList.remove('hidden');
        modeSelectionModal.classList.add('hidden');
        syncDataBtn.classList.add('hidden');
        navigateTo('dashboard');
        updateDashboardStats();
    }
    ...
    async function handleSyncData() {
        const changesToSync = Object.values(pendingChanges.updated);
        const changeCount = changesToSync.length;
        ...
        if (!confirm(`You have ${changeCount} unsynced change(s). Do you want to sync them with the server now?`)) {
            return;
        }
        ...
        try {
            const updatePromises = changesToSync.map(recordData => {
                const recordId = recordData.id;
                // **FIX**: Create a clean payload, removing read-only fields
                const payload = { ...recordData };
                delete payload.id;
                delete payload.batch_name;
                delete payload.event_names; 
                return updateRecord(recordId, payload);
            });
            
            await Promise.all(updatePromises);

            alert("Data synced successfully! The application will now reload the fresh data.");

            // Reset pending changes and reload all data from server
            pendingChanges = { updated: {} };
            updateSyncStatus();
            await handleImportMode(); // This will re-fetch everything
            ...
            syncDataBtn.innerHTML = `Sync Data <span id="sync-status-indicator" class="absolute top-0 right-0 -mt-2 -mr-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center"></span>`;
            updateSyncStatus(); // Update the counter
        }
    }

    function updateSyncStatus() {
        const indicator = document.getElementById('sync-status-indicator');
        const changeCount = Object.keys(pendingChanges.updated).length;
        if (changeCount > 0) {
            indicator.textContent = changeCount;
            indicator.classList.remove('hidden');
            syncDataBtn.classList.add('pulse-sync');
        } else {
            indicator.classList.add('hidden');
            syncDataBtn.classList.remove('pulse-sync');
        }
    }
    ...
    async function handleSearch(e, pageOrUrl = 1) { if (e) e.preventDefault(); if (!searchResultsContainer) return; searchResultsContainer.innerHTML = '<p class="text-gray-500">Searching...</p>'; const params = { naam: document.getElementById('search-name').value, voter_no: document.getElementById('search-voter-no').value, pitar_naam: document.getElementById('search-father-name').value, thikana: document.getElementById('search-address').value, matar_naam: document.getElementById('search-mother-name').value, kromik_no: document.getElementById('search-kromik-no').value, pesha: document.getElementById('search-profession').value, phone_number: document.getElementById('search-phone').value, }; if (currentDataMode === 'direct') { let searchParams; if (typeof pageOrUrl === 'string') { searchParams = pageOrUrl; } else { const apiParams = { naam__icontains: params.naam, voter_no: params.voter_no, pitar_naam__icontains: params.pitar_naam, thikana__icontains: params.thikana, matar_naam__icontains: params.matar_naam, kromik_no: params.kromik_no, pesha__icontains: params.pesha, phone_number__icontains: params.phone_number, }; searchParams = Object.fromEntries(Object.entries(apiParams).filter(([_, v]) => v && v.trim() !== '')); } try { const data = await searchRecords(searchParams); originalRecords = data.results; displaySearchResults(data.results); displayPaginationControls(searchPaginationContainer, data.previous, data.next, (nextUrl) => handleSearch(null, nextUrl)); } catch (error) { searchResultsContainer.innerHTML = `<p class="text-red-500">${error.message}</p>`; } } else { const filtered = filterImportedRecords(params); const page = typeof pageOrUrl === 'number' ? pageOrUrl : 1; const pageSize = 50; const start = (page - 1) * pageSize; const end = start + pageSize; const paginatedResults = filtered.slice(start, end); originalRecords = paginatedResults; displaySearchResults(paginatedResults); displayClientSidePagination(searchPaginationContainer, page, filtered.length, pageSize, (nextPage) => handleSearch(null, nextPage)); } }
    ...
    async function handleAllDataFileSelect(pageOrUrl = 1) { if (!allDataBatchSelect || !allDataFileSelect || !allDataTableContainer) return; const batchId = allDataBatchSelect.value; const fileName = allDataFileSelect.value; allDataTableContainer.innerHTML = '<p class="p-4 text-gray-500">Loading records...</p>'; if (!batchId) return; if (currentDataMode === 'direct') { let params; if (typeof pageOrUrl === 'string') { params = pageOrUrl; } else { currentAllDataParams = { batch: batchId }; if (fileName && fileName !== 'all') { currentAllDataParams.file_name = fileName; } params = currentAllDataParams; } try { const data = await searchRecords(params); originalRecords = data.results; renderReadOnlyTable(data.results); displayPaginationControls(allDataPaginationContainer, data.previous, data.next, (nextUrl) => handleAllDataFileSelect(nextUrl)); } catch (error) { allDataTableContainer.innerHTML = `<p class="p-4 text-red-500">${error.message}</p>`; } } else { const params = { batch: batchId }; if (fileName && fileName !== 'all') { params.file_name = fileName; } const filtered = filterImportedRecords(params); const page = typeof pageOrUrl === 'number' ? pageOrUrl : 1; const pageSize = 50; const start = (page - 1) * pageSize; const end = start + pageSize; const paginatedResults = filtered.slice(start, end); originalRecords = paginatedResults; renderReadOnlyTable(paginatedResults); displayClientSidePagination(allDataPaginationContainer, page, filtered.length, pageSize, (nextPage) => handleAllDataFileSelect(nextPage)); } }
    ...
    async function handleModalSave() {
        ...
        const selectedEventIds = Array.from(document.querySelectorAll('#edit-events-checkboxes input:checked')).map(cb => parseInt(cb.value, 10));
        
        const activePageElement = document.querySelector('.nav-link.active');
        if (!activePageElement) return;
        const activePageId = activePageElement.id.split('-')[1];
        const statusContainer = pages[activePageId].querySelector('.space-y-4, #alldata-status');


        if (currentDataMode === 'direct') {
            if(statusContainer) statusContainer.innerHTML = `<p class="text-blue-600">Saving record ${recordId}...</p>`;
            try {
                await updateRecord(recordId, { ...updatedData, events: selectedEventIds });
                if(statusContainer) statusContainer.innerHTML = `<p class="text-green-600">Successfully saved record ${recordId}!</p>`;
                editRecordModal.classList.add('hidden');
                
                if (activePageId === 'alldata') handleAllDataFileSelect();
                else if (activePageId === 'search') handleSearch(null);

            } catch (error) {
                if(statusContainer) statusContainer.innerHTML = `<p class="text-red-500">Error saving changes: ${error.message}</p>`;
            }
        } else { // Import Mode Logic
            const recordIndex = allImportedRecords.findIndex(r => r.id == recordId);
            if (recordIndex === -1) {
                alert("Error: could not find the record in the local data. Please sync and try again.");
                return;
            }

            const selectedEventNames = selectedEventIds.map(id => allEventsMap[id]?.name || 'Unknown Event');
            const locallyUpdatedRecord = { 
            ...
            allImportedRecords[recordIndex] = locallyUpdatedRecord;
            pendingChanges.updated[recordId] = locallyUpdatedRecord;
            
            editRecordModal.classList.add('hidden');
            updateSyncStatus();
            if(statusContainer) statusContainer.innerHTML = `<p class="text-green-600">Record for '${updatedData.naam}' updated locally. Click 'Sync Data' to save.</p>`;
    ...
    function displaySearchResults(results) { if (!searchResultsContainer) return; searchResultsContainer.innerHTML = ''; if (!results || results.length === 0) { searchResultsContainer.innerHTML = '<p class="text-gray-600">No results found.</p>'; return; } results.forEach(record => { const card = document.createElement('div'); card.className = 'bg-white p-4 rounded-lg shadow-md flex space-x-4 items-start'; const safeText = (text) => text || '<span class="text-gray-400">N/A</span>'; card.innerHTML = ` <img src="${record.photo_link}" alt="Voter Photo" class="w-24 h-24 rounded-md object-cover" onerror="this.onerror=null;this.src='https://placehold.co/100x100/EEE/31343C?text=No+Image';"> <div class="flex-1"> <div class="flex justify-between items-start"> <h3 class="text-lg font-bold text-gray-800">${safeText(record.naam)}</h3> <button class="edit-btn text-blue-500 hover:text-blue-700" data-record-id="${record.id}"> <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 pointer-events-none" viewBox="0 0 20 20" fill="currentColor"> <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /> <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /> </svg> </button> </div> <p class="text-sm text-gray-500">Voter No: ${safeText(record.voter_no)}</p> <div class="mt-2 text-sm space-y-1 text-gray-600"> <p><strong>Father's Name:</strong> ${safeText(record.pitar_naam)}</p> <p><strong>Address:</strong> ${safeText(record.thikana)}</p> <p><strong>Relationship:</strong> ${safeText(record.relationship_status)}</p> <p><strong>Events:</strong> ${safeText(record.event_names?.join(', '))}</p> </div> </div> `; searchResultsContainer.appendChild(card); }); searchResultsContainer.querySelectorAll('.edit-btn').forEach(btn => btn.addEventListener('click', (e) => openEditModal(e.currentTarget.dataset.recordId))); }
    
    function showLogin() { loginScreen.classList.remove('hidden'); appContainer.classList.add('hidden'); modeSelectionModal.classList.add('hidden'); }
    function showApp() { loginScreen.classList.add('hidden'); appContainer.classList.add('hidden'); modeSelectionModal.classList.remove('hidden'); }
    function debounce(func, delay) { let timeout; return function(...args) { const context = this; clearTimeout(timeout); timeout = setTimeout(() => func.apply(context, args), delay); }; }
    function filterImportedRecords(params) { const lowerCaseParams = {}; for (const key in params) { if (typeof params[key] === 'string' && key !== 'batch') { lowerCaseParams[key] = params[key].toLowerCase(); } else { lowerCaseParams[key] = params[key]; } } return allImportedRecords.filter(r => { const nameMatch = !lowerCaseParams.naam || (r.naam && r.naam.toLowerCase().includes(lowerCaseParams.naam)); const voterNoMatch = !lowerCaseParams.voter_no || r.voter_no === lowerCaseParams.voter_no; const pitarNaamMatch = !lowerCaseParams.pitar_naam || (r.pitar_naam && r.pitar_naam.toLowerCase().includes(lowerCaseParams.pitar_naam)); const thikanaMatch = !lowerCaseParams.thikana || (r.thikana && r.thikana.toLowerCase().includes(lowerCaseParams.thikana)); const matarNaamMatch = !lowerCaseParams.matar_naam || (r.matar_naam && r.matar_naam.toLowerCase().includes(lowerCaseParams.matar_naam)); const kromikNoMatch = !lowerCaseParams.kromik_no || r.kromik_no === lowerCaseParams.kromik_no; const peshaMatch = !lowerCaseParams.pesha || (r.pesha && r.pesha.toLowerCase().includes(lowerCaseParams.pesha)); const phoneMatch = !lowerCaseParams.phone_number || (r.phone_number && r.phone_number.includes(lowerCaseParams.phone_number)); const batchMatch = !lowerCaseParams.batch || r.batch == lowerCaseParams.batch; const fileNameMatch = !lowerCaseParams.file_name || r.file_name === lowerCaseParams.file_name; const relationshipMatch = !lowerCaseParams.relationship_status || r.relationship_status === lowerCaseParams.relationship_status; return nameMatch && voterNoMatch && pitarNaamMatch && thikanaMatch && matarNaamMatch && kromikNoMatch && peshaMatch && phoneMatch && batchMatch && fileNameMatch && relationshipMatch; }); }
    
    async function openEditModal(recordId) { const sourceData = currentDataMode === 'import' ? allImportedRecords : originalRecords; const record = sourceData.find(r => r.id == recordId); if (!record) { alert('Could not find record details.'); return; } editRecordIdInput.value = record.id; document.getElementById('edit-naam').value = record.naam || ''; document.getElementById('edit-voter-no').value = record.voter_no || ''; document.getElementById('edit-kromik-no').value = record.kromik_no || ''; document.getElementById('edit-jonmo-tarikh').value = record.jonmo_tarikh || ''; document.getElementById('edit-gender').value = record.gender || ''; document.getElementById('edit-pitar-naam').value = record.pitar_naam || ''; document.getElementById('edit-matar-naam').value = record.matar_naam || ''; document.getElementById('edit-pesha').value = record.pesha || ''; document.getElementById('edit-occupation-details').value = record.occupation_details || ''; document.getElementById('edit-phone-number').value = record.phone_number || ''; document.getElementById('edit-whatsapp-number').value = record.whatsapp_number || ''; document.getElementById('edit-facebook-link').value = record.facebook_link || ''; document.getElementById('edit-photo-link').value = record.photo_link || ''; document.getElementById('edit-thikana').value = record.thikana || ''; document.getElementById('edit-description').value = record.description || ''; document.getElementById('edit-political-status').value = record.political_status || ''; document.getElementById('edit-relationship-status').value = record.relationship_status || 'Regular'; const eventsContainer = document.getElementById('edit-events-checkboxes'); eventsContainer.innerHTML = 'Loading events...'; try { const allEventsResponse = await getEvents(); const allEvents = allEventsResponse.results; allEventsMap = allEvents.reduce((map, event) => { map[event.id] = event; return map; }, {}); eventsContainer.innerHTML = ''; allEvents.forEach(event => { const isChecked = record.event_names.includes(event.name); const checkboxDiv = document.createElement('div'); checkboxDiv.className = 'flex items-center'; checkboxDiv.innerHTML = ` <input id="event-${event.id}" name="events" type="checkbox" value="${event.id}" class="h-4 w-4 text-indigo-600 border-gray-300 rounded" ${isChecked ? 'checked' : ''}> <label for="event-${event.id}" class="ml-2 block text-sm text-gray-900">${event.name}</label> `; eventsContainer.appendChild(checkboxDiv); }); } catch (error) { eventsContainer.innerHTML = '<p class="text-red-500">Could not load events.</p>'; } editRecordModal.classList.remove('hidden'); }
    ...
    async function handleRecalculateAges() { if (!ageRecalculationStatus) return; ageRecalculationStatus.innerHTML = '<p class="text-blue-600">Recalculating ages...</p>'; try { const result = await recalculateAllAges(); ageRecalculationStatus.innerHTML = `<p class="text-green-600">${result.message}</p>`; initializeAgeManagementPage(); } catch (error) { ageRecalculationStatus.innerHTML = `<p class="text-red-600">Error: ${error.message}</p>`; } }
    async function handleFamilyTreeSearch(event) { const input = event.target; const query = input.value.trim(); const isMainSearch = input.id === 'family-main-search'; const resultsContainer = isMainSearch ? familyMainSearchResults : familyRelativeSearchResults; if (!query) { resultsContainer.innerHTML = ''; return; } if (currentDataMode === 'direct') { try { const data = await searchRecords({ naam__icontains: query, page_size: 10 }); resultsContainer.innerHTML = ''; if (data.results.length === 0) { resultsContainer.innerHTML = '<p class="text-gray-500">No results found.</p>'; } else { data.results.forEach(record => { const button = document.createElement('button'); button.className = 'block w-full text-left p-2 rounded hover:bg-gray-100'; button.textContent = `${record.naam} (Voter No: ${record.voter_no || 'N/A'})`; button.onclick = () => { if (isMainSearch) { selectMainPerson(record); } else { selectRelative(record); } }; resultsContainer.appendChild(button); }); } } catch (error) { resultsContainer.innerHTML = `<p class="text-red-500">${error.message}</p>`; } } else { const filtered = filterImportedRecords({ naam: query }).slice(0, 10); resultsContainer.innerHTML = ''; if (filtered.length === 0) { resultsContainer.innerHTML = '<p class="text-gray-500">No results found.</p>'; } else { filtered.forEach(record => { const button = document.createElement('button'); button.className = 'block w-full text-left p-2 rounded hover:bg-gray-100'; button.textContent = `${record.naam} (Voter No: ${record.voter_no || 'N/A'})`; button.onclick = () => { if (isMainSearch) { selectMainPerson(record); } else { selectRelative(record); } }; resultsContainer.appendChild(button); }); } } }
    ...
    async function handleCallHistorySearch(event) { const query = event.target.value.trim(); if (!query) { callHistorySearchResults.innerHTML = ''; return; } if (currentDataMode === 'direct') { try { const data = await searchRecords({ naam__icontains: query, page_size: 10 }); callHistorySearchResults.innerHTML = ''; if (data.results.length === 0) { callHistorySearchResults.innerHTML = '<p class="text-gray-500">No results found.</p>'; } else { data.results.forEach(record => { const button = document.createElement('button'); button.className = 'block w-full text-left p-2 rounded hover:bg-gray-100'; button.textContent = `${record.naam} (Voter No: ${record.voter_no || 'N/A'})`; button.onclick = () => selectPersonForCallHistory(record); callHistorySearchResults.appendChild(button); }); } } catch (error) { callHistorySearchResults.innerHTML = `<p class="text-red-500">${error.message}</p>`; } } else { const filtered = filterImportedRecords({ naam: query }).slice(0, 10); callHistorySearchResults.innerHTML = ''; if (filtered.length === 0) { callHistorySearchResults.innerHTML = '<p class="text-gray-500">No results found.</p>'; } else { filtered.forEach(record => { const button = document.createElement('button'); button.className = 'block w-full text-left p-2 rounded hover:bg-gray-100'; button.textContent = `${record.naam} (Voter No: ${record.voter_no || 'N/A'})`; button.onclick = () => selectPersonForCallHistory(record); callHistorySearchResults.appendChild(button); }); } } }
    ...
    async function handleDeleteEvent(eventId) { if (!confirm('Are you sure you want to delete this event?')) return; try { await deleteEvent(eventId); initializeEventsPage(); } catch (error) { alert(error.message); } }
    async function handleFilterByEvent(url = null) { const eventId = eventFilterSelect.value; if (!eventId || isNaN(parseInt(eventId))) { eventFilterResults.innerHTML = '<p class="text-gray-600">Please select an event.</p>'; return; } eventFilterResults.innerHTML = '<p class="text-gray-500">Loading records...</p>'; try { const data = await getRecordsForEvent(eventId, url); displayEventRecords(data.results); displayPaginationControls(eventFilterPagination, data.previous, data.next, (nextUrl) => handleFilterByEvent(nextUrl)); } catch (error) { eventFilterResults.innerHTML = `<p class="text-red-500">${error.message}</p>`; } }
    ...
    function renderReadOnlyTable(records) { if (!allDataTableContainer) return; allDataTableContainer.innerHTML = ''; if (!records || records.length === 0) { allDataTableContainer.innerHTML = '<p class="p-4 text-gray-600">No records found.</p>'; return; } const table = document.createElement('table'); table.className = 'min-w-full divide-y divide-gray-200'; table.innerHTML = ` <thead class="bg-gray-50"> <tr> <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th> <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Voter No</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Address</th> <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Events</th> <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th> </tr> </thead> <tbody class="bg-white divide-y divide-gray-200"> </tbody> `; const tbody = table.querySelector('tbody'); records.forEach(record => { const row = document.createElement('tr'); row.dataset.recordId = record.id; row.className = 'cursor-pointer hover:bg-gray-50'; row.innerHTML = ` <td class="px-6 py-4 whitespace-nowrap">${record.naam || ''}</td> <td class="px-6 py-4 whitespace-nowrap">${record.voter_no || ''}</td><td class="px-6 py-4 whitespace-nowrap">${record.thikana || ''}</td> <td class="px-6 py-4 whitespace-nowrap">${record.event_names?.join(', ') || ''}</td> <td class="px-6 py-4 whitespace-nowrap"> <button data-record-id="${record.id}" class="edit-btn text-indigo-600 hover:text-indigo-900">Edit</button> </td> `; tbody.appendChild(row); }); allDataTableContainer.appendChild(table); }
    function displayRelationshipList(status, pageOrUrl = 1) { if (!relContentContainer || !relPaginationContainer) return; relContentContainer.innerHTML = '<p class="text-gray-500">Loading...</p>'; relPaginationContainer.innerHTML = ''; if (currentDataMode === 'direct') { const params = { relationship_status: status }; const url = typeof pageOrUrl === 'string' ? pageOrUrl : null; searchRecords(url || params).then(data => { if (!data.results || data.results.length === 0) { relContentContainer.innerHTML = `<p class="text-gray-600">No records found with status: ${status}.</p>`; return; } const listContainer = document.createElement('div'); listContainer.className = 'space-y-4'; data.results.forEach(record => { const card = document.createElement('div'); card.className = 'result-card'; card.innerHTML = ` <h3>${record.naam}</h3> <p><strong>Voter No:</strong> ${record.voter_no || 'N/A'}</p><p><strong>Address:</strong> ${record.thikana || 'N/A'}</p>`; listContainer.appendChild(card); }); relContentContainer.innerHTML = ''; relContentContainer.appendChild(listContainer); displayPaginationControls(relPaginationContainer, data.previous, data.next, (nextUrl) => displayRelationshipList(status, nextUrl)); }).catch(error => { relContentContainer.innerHTML = `<p class="text-red-500">${error.message}</p>`; }); } else { const page = typeof pageOrUrl === 'number' ? pageOrUrl : 1; const filtered = filterImportedRecords({ relationship_status: status }); const pageSize = 50; const start = (page - 1) * pageSize; const end = start + pageSize; const paginatedResults = filtered.slice(start, end); if (paginatedResults.length === 0) { relContentContainer.innerHTML = `<p class="text-gray-600">No records found with status: ${status}.</p>`; return; } const listContainer = document.createElement('div'); listContainer.className = 'space-y-4'; paginatedResults.forEach(record => { const card = document.createElement('div'); card.className = 'result-card'; card.innerHTML = `<h3>${record.naam}</h3><p><strong>Voter No:</strong> ${record.voter_no || 'N/A'}</p><p><strong>Address:</strong> ${record.thikana || 'N/A'}</p>`; listContainer.appendChild(card); }); relContentContainer.innerHTML = ''; relContentContainer.appendChild(listContainer); displayClientSidePagination(relPaginationContainer, page, filtered.length, pageSize, (nextPage) => displayRelationshipList(status, nextPage)); } }
    async function displayRelationshipStats() { if (!relContentContainer || !relPaginationContainer) return; relContentContainer.innerHTML = '<p class="text-gray-500">Loading stats...</p>'; relPaginationContainer.innerHTML = ''; try { const stats = await getRelationshipStats(); let byBatchHtml = '<h3>Distribution by Batch</h3><div class="space-y-4 mt-4">'; const batchData = stats.by_batch.reduce((acc, item) => { if (!acc[item.batch__name]) { acc[item.batch__name] = {}; } acc[item.batch__name][item.relationship_status] = item.count; return acc; }, {}); for (const batchName in batchData) { const counts = batchData[batchName]; byBatchHtml += `<div class="p-4 border rounded-lg"><h4 class="font-bold">${batchName}</h4><div class="flex justify-center space-x-4 mt-2 items-end"><div class="text-center"><div class="bg-green-500 text-white text-xs py-1 flex items-center justify-center rounded-t-md" style="height:${(counts.Friend||0)*10+20}px;width:60px;">${counts.Friend||0}</div><div class="text-xs mt-1">Friend</div></div><div class="text-center"><div class="bg-red-500 text-white text-xs py-1 flex items-center justify-center rounded-t-md" style="height:${(counts.Enemy||0)*10+20}px;width:60px;">${counts.Enemy||0}</div><div class="text-xs mt-1">Enemy</div></div><div class="text-center"><div class="bg-yellow-500 text-white text-xs py-1 flex items-center justify-center rounded-t-md" style="height:${(counts.Connected||0)*10+20}px;width:60px;">${counts.Connected||0}</div><div class="text-xs mt-1">Connected</div></div></div></div>`; } byBatchHtml += '</div>'; relContentContainer.innerHTML = byBatchHtml; } catch (error) { relContentContainer.innerHTML = `<p class="text-red-500">${error.message}</p>`; } }
    ...
    async function initializeAnalysisPage() { const contentEl = document.getElementById('analysis-content'); if (!contentEl) return; contentEl.innerHTML = '<p class="text-gray-500">Loading analysis...</p>'; try { const stats = await getAnalysisStats(); contentEl.innerHTML = ''; renderProfessionChart(stats.professions); renderGenderChart(stats.genders); renderAgeChart(stats.age_groups, 'age-chart-container'); } catch (error) { contentEl.innerHTML = `<p class="text-red-500">Failed to load analysis: ${error.message}</p>`; } }
    ...
    function renderProfessionChart(professionData) { const container = document.getElementById('profession-chart-container'); if (!container || !professionData || professionData.length === 0) { if (container) container.innerHTML = '<p class="text-gray-500">No profession data.</p>'; return; } const canvas = container.getContext('2d'); if (Chart.getChart(canvas)) Chart.getChart(canvas).destroy(); const labels = professionData.map(p => p.pesha); const data = professionData.map(p => p.count); new Chart(canvas, { type: 'doughnut', data: { labels: labels, datasets: [{ label: 'Professions', data: data, backgroundColor: ['#4F46E5', '#7C3AED', '#EC4899', '#F59E0B', '#10B981', '#3B82F6', '#6366F1', '#D946EF', '#FBBF24', '#34D399'], hoverOffset: 4 }] }, options: { responsive: true, plugins: { legend: { position: 'top' }, title: { display: true, text: 'Voter Distribution by Profession' } } } }); }
    function renderGenderChart(genderData) { const container = document.getElementById('gender-chart-container'); if (!container || !genderData || genderData.length === 0) { if (container) container.innerHTML = '<p class="text-gray-500">No gender data.</p>'; return; } const canvas = container.getContext('2d'); if (Chart.getChart(canvas)) Chart.getChart(canvas).destroy(); const labels = genderData.map(g => g.gender); const data = genderData.map(g => g.count); new Chart(canvas, { type: 'pie', data: { labels: labels, datasets: [{ label: 'Genders', data: data, backgroundColor: ['#3B82F6', '#EC4899', '#6B7280'], hoverOffset: 4 }] }, options: { responsive: true, plugins: { legend: { position: 'top' }, title: { display: true, text: 'Voter Distribution by Gender' } } } }); }
    function renderAgeChart(ageData, containerId) { const container = document.getElementById(containerId); if (!container || !ageData) { if (container) container.innerHTML = '<p class="text-gray-500">No age data.</p>'; return; } const canvas = container.getContext('2d'); if (Chart.getChart(canvas)) Chart.getChart(canvas).destroy(); const labels = ['18-25', '26-35', '36-45', '46-60', '60+']; const data = [ageData.group_18_25, ageData.group_26_35, ageData.group_36_45, ageData.group_46_60, ageData.group_60_plus]; new Chart(canvas, { type: 'bar', data: { labels: labels, datasets: [{ label: 'Number of Voters', data: data, backgroundColor: '#10B981' }] }, options: { responsive: true, plugins: { legend: { display: false }, title: { display: true, text: 'Voter Distribution by Age Group' } }, scales: { y: { beginAtZero: true } } } }); }
